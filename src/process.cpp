...
namespace {
    void exit_with_perror(
        tdb::pipe& channel, std::string const& prefix) {
...
std::unique_ptr<tdb::process> tdb::process::launch(std::filesystem::path path, bool debug) {
...
std::unique_ptr<tdb::process> tdb::process::attach(pid_t pid) {
    if (pid == 0) {
